...
const imageSendingHandler = async (file) => {
  const receiverId = currentChat.members.find(
    (member) => member !== seller?._id // or user?._id in UserInbox
  );

  const formData = new FormData();
  formData.append("image", file);
  formData.append("sender", seller?._id || user?._id);
  ...
  <>
    {Array.isArray(item.images) ? (
      ...
      <img
        src={
          item.images.url
            ? `${backend_url}${item.images.url}`
            : item.images.startsWith("data:")
            ? item.images
            : `${backend_url}/${item.images}`
        }
